...
class NetworkInterface {
  private:
    ...
    // ip -> <映射建立时间，mac地址>
    std::unordered_map<uint32_t, std::pair<size_t, EthernetAddress>> _arp_cache;
    // ip -> 上次给这个ip发arp包的时间
    std::unordered_map<uint32_t, size_t> _ip_timer;
    size_t _cur_time;

    std::multimap<uint32_t, InternetDatagram> _wait_frames;
  public:
